...
E. ✅ 해소(51eb88d+bd4fea9) — site-safety completionApprovedAt 삼중결함(SQL CASE+컬럼+프론트 바인딩) 전부 수정. E2E 26P0F.
...
- 2026-06-22 [발견-정정·lab2] ★위 '이중결함' → 실제 **삼중결함**: 프론트 SiteSafetyReportTab이 completionApprovedAt 미바인딩(완료일=modifiedAt 프록시)이라 백엔드를 채워도 화면 미노출. 형제 6도메인(Audit·Permit·Emr·Risk·Contractor·KPI)은 바인딩 보유 → SiteSafety만 누락. 다음 형제/신규 도메인 점검 단서.
- 2026-06-22 [수정·lab2] site-safety completionApprovedAt 삼중결함 해소 — 백엔드 3계층(SiteSafetyApprovalSplitInitializer 컬럼2 + transition COMPLETION CASE + resultMap, 51eb88d) + 프론트 레포트 바인딩(형제패턴 fallback, bd4fea9). E2E 25P1F→26P0F. 무회귀: 공용 approved_at 미터치·백필 없음. 위 [발견-버그] 해소.
- 2026-06-22 [메모·lab2] PSM 복구실험(exp/psm-restore, 폐기됨) 중 발견 — PsmTablesInitializer @Order(검증 동작상)가 PersonRefColumnsInitializer(@Order 50)보다 뒤라, 신규 부팅 시 psm 테이블에 PersonRef 컬럼(created_by/modified_by/plan_approver/completion_approver) 미생성(테이블 부재로 ALTER 4902 WARN) → 2부팅 필요(2차 기동 시 멱등 자가해소·CREATE까지 정상 실증). 향후 PSM 정식 도입 시 PsmTablesInitializer Order를 PersonRef 앞으로(또는 PsmTables가 컬럼까지 생성). 원본 PSM도 동일 잠재결함 추정. ※복구는 fbd1149^ 34파일+공용10 재삽입+YescoPsm(daab65e^)으로 가능, self-contained(화학/환경 무관) 확인. DB는 V204-206 DDL 생존+bak20260613_백업 6 잔존.
