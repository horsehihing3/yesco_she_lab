...
import {
  ...
  TableContainer, Pagination, TextField, Button,
  IconButton, Alert,
...
const emptyForm = { itemName: '', itemCode: '', version: '', issueDate: '', retireDate: '', retireReason: '', language: '', fileSize: '', msdsType: 'PRODUCT' as const, isLatest: false }

const MsdsProductOldTab: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['msds-product-old', page],
    queryFn: () => msdsApi.getByType('PRODUCT', false, page, 10),
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['msds-product-old'] })
  ...
  const handleOpenCreate = () => { setSelectedItem(null); setForm({ ...emptyForm, issueDate: todayStr(), retireDate: todayStr() }); setViewMode('create') }
  const handleOpenEdit = (item: Msds) => {
    setSelectedItem(item)
    setForm({ itemName: item.itemName || '', itemCode: item.itemCode || '', version: item.version || '', issueDate: item.issueDate || '', retireDate: item.retireDate || '', retireReason: item.retireReason || '', language: item.language || '', fileSize: item.fileSize || '', msdsType: 'PRODUCT', isLatest: false })
    setViewMode('edit')
  }
  ...
  // DEV ONLY — 비어있는 항목을 제품 MSDS 구버전 더미데이터로 채움 (입력값 보존)
  const fillTestData = () => setForm(prev => ({
    ...
    retireDate: prev.retireDate || todayStr(),
    retireReason: prev.retireReason || '신규 버전(2.1) 발행으로 폐기',
  }))
      ...
      <Box>
        ...
        <Box sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden', mb: 3 }}>
            <Box sx={rowSx}>
              ...
              <Typography sx={labelSx}>{t('chem.msds.itemCode', '제품코드')}</Typography>
              <Box sx={valSx}><Typography variant="body2">{selectedItem.itemCode || ''}</Typography></Box>
            </Box>
            <Box sx={rowSx}>
            ...
            <Box sx={rowSx}>
              <Typography sx={labelSx}>{t('chem.msds.retireDate', '폐기일')}</Typography>
              <Box sx={valBorderSx}><Typography variant="body2">{selectedItem.retireDate || ''}</Typography></Box>
              <Typography sx={labelSx}>{t('chem.msds.retireReason', '폐기 사유')}</Typography>
              <Box sx={valSx}><Typography variant="body2">{selectedItem.retireReason || ''}</Typography></Box>
            </Box>
            <Box sx={{ display: 'flex' }}>
              <Typography sx={labelSx}>{t('chem.msds.language', '언어')}</Typography>
              <Box sx={valBorderSx}><Typography variant="body2">{selectedItem.language || ''}</Typography></Box>
              <Typography sx={labelSx}>{t('chem.msds.fileSize', '파일 크기')}</Typography>
              <Box sx={valSx}><Typography variant="body2">{selectedItem.fileSize || ''}</Typography></Box>
            </Box>
          </Box>
        {/* Mobile */}
        <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 2, mb: 3 }}>
          {[
            ...
            [t('chem.msds.itemCode', '제품코드'), selectedItem.itemCode],
            [t('chem.msds.version', '버전'), selectedItem.version],
            [t('chem.msds.issueDate', '발행일'), selectedItem.issueDate],
            [t('chem.msds.retireDate', '폐기일'), selectedItem.retireDate],
            [t('chem.msds.retireReason', '폐기 사유'), selectedItem.retireReason],
      ...
      <Box>
        ...
        <Paper sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden', mb: 2 }}>
          <Box sx={rowSx}>
            ...
            <Typography sx={labelSx}>{t('chem.msds.itemCode', '제품코드')}</Typography>
            <Box sx={valSx}><TextField fullWidth size="small" value={form.itemCode} onChange={e => setForm({ ...form, itemCode: e.target.value })} /></Box>
          </Box>
          <Box sx={rowSx}>
            <Typography sx={labelSx}>{t('chem.msds.version', '버전')}</Typography>
          ...
          <Box sx={rowSx}>
            <Typography sx={labelSx}>{t('chem.msds.retireDate', '폐기일')}</Typography>
            <Box sx={valBorderSx}><DatePickerField value={form.retireDate} onChange={(v: string) => setForm({ ...form, retireDate: v })} size="small" fullWidth /></Box>
            <Typography sx={labelSx}>{t('chem.msds.retireReason', '폐기 사유')}</Typography>
            <Box sx={valSx}><TextField fullWidth size="small" value={form.retireReason} onChange={e => setForm({ ...form, retireReason: e.target.value })} /></Box>
          </Box>
          <Box sx={{ display: 'flex' }}>
            <Typography sx={labelSx}>{t('chem.msds.language', '언어')}</Typography>
            <Box sx={valBorderSx}><Autocomplete
  multiple
  size="small"
  options={langOptions}
...
/></Box>
            <Typography sx={labelSx}>{t('chem.msds.fileSize', '파일 크기')}</Typography>
            <Box sx={valSx}><TextField fullWidth size="small" value={form.fileSize} onChange={e => setForm({ ...form, fileSize: e.target.value })} /></Box>
          </Box>
        </Paper>
        {/* Mobile Form */}
        <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 2, mb: 2 }}>
          ...
          <Box>
            <Typography variant="body2" fontWeight="bold" sx={{ mb: 0.5, bgcolor: 'grey.200', px: 1.5, py: 0.75, borderRadius: 0.5 }}>{t('chem.msds.itemCode', '제품코드')}</Typography>
            <TextField size="small" fullWidth value={form.itemCode} onChange={e => setForm({ ...form, itemCode: e.target.value })} />
          </Box>
          <Box>
          ...
          <Box>
            <Typography variant="body2" fontWeight="bold" sx={{ mb: 0.5, bgcolor: 'grey.200', px: 1.5, py: 0.75, borderRadius: 0.5 }}>{t('chem.msds.retireDate', '폐기일')}</Typography>
            <DatePickerField value={form.retireDate} onChange={(v: string) => setForm({ ...form, retireDate: v })} size="small" fullWidth />
          </Box>
          <Box>
            <Typography variant="body2" fontWeight="bold" sx={{ mb: 0.5, bgcolor: 'grey.200', px: 1.5, py: 0.75, borderRadius: 0.5 }}>{t('chem.msds.retireReason', '폐기 사유')}</Typography>
            <TextField size="small" fullWidth value={form.retireReason} onChange={e => setForm({ ...form, retireReason: e.target.value })} />
          </Box>
          <Box>
  ...
  return (
    <Box>
      <Alert severity="warning" sx={{ mb: 2 }}>
        {t('chem.msds.oldWarning', '구 버전은 참조 목적으로만 보관됩니다.')}
      </Alert>

      {/* Search - PC */}
      <Box sx={{ display: { xs: 'none', md: 'flex' }, justifyContent: 'space-between', alignItems: 'center', mb: 2, gap: 1 }}>
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          <ListSearchBar placeholder={t('chem.msds.searchProductPlaceholder')}
      ...
      <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 1, mb: 2 }}>
        <ListSearchBar fullWidth placeholder={t('chem.msds.searchProductPlaceholder')}
          value={keywordInput} onChange={setKeywordInput} onSearch={applySearch} />
      ...
      <TableContainer component={Paper} sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'divider', overflowX: 'auto' }}>
        <Table size="small" sx={{ '& .MuiTableCell-root': { borderRight: '1px solid', borderColor: 'divider' }, '& .MuiTableCell-root:last-child': { borderRight: 'none' } }}>
          <TableHead>
            <TableRow>
              <TableCell align="center" sx={headerCellSx}>{t('chem.msds.itemCode', '제품코드')}</TableCell>
              ...
              <TableCell align="center" sx={headerCellSx}>{t('chem.msds.retireDate', '폐기일')}</TableCell>
              <TableCell align="center" sx={headerCellSx}>{t('chem.msds.retireReason', '폐기 사유')}</TableCell>
              <TableCell align="center" sx={headerCellSx}>{t('chem.msds.language', '언어')}</TableCell>
              <TableCell align="center" sx={lastHeaderCellSx}>{t('chem.msds.fileSize', '파일 크기')}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {items.length > 0 ? items.map((item) => (
              <TableRow key={item.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
                <TableCell align="center" sx={cellBorderSx}>{item.itemCode || ''}</TableCell>
                ...
                <TableCell align="center" sx={cellBorderSx}>{item.retireDate || ''}</TableCell>
                <TableCell sx={cellBorderSx}>{item.retireReason || ''}</TableCell>
        ...
        {items.length > 0 ? items.map((item) => (
          <Paper key={item.id} sx={{ p: 2, cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
            ...
            <Typography variant="caption" color="text.secondary">
              {item.itemCode || ''} | {t('chem.msds.retireDate', '폐기일')}: {item.retireDate || ''}
            </Typography>
          </Paper>
        )) : (
...
export default MsdsProductOldTab
