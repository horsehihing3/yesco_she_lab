...
const MsdsProductLatestTab: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['msds-product-latest', page],
    queryFn: () => msdsApi.getByType('PRODUCT', true, page, 10),
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['msds-product-latest'] })
  ...
  const handleOpenEdit = (item: Msds) => {
    ...
    setForm({ itemName: item.itemName || '', itemCode: item.itemCode || '', version: item.version || '', issueDate: item.issueDate || '', language: item.language || '', exportCountries: item.exportCountries || '', fileSize: item.fileSize || '', status: item.status || 'VALID', msdsType: 'PRODUCT', isLatest: true })
    setViewMode('edit')
  }
  ...
  // DEV ONLY — 비어있는 항목을 제품 MSDS 최신본 더미데이터로 채움 (입력값 보존)
  const fillTestData = () => setForm(prev => ({
    ...
    exportCountries: prev.exportCountries || '미국 / 일본 / 베트남',
  }))
      ...
      <Box>
        ...
        <Box sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden', mb: 3 }}>
            ...
            <Box sx={rowSx}>
              ...
              <Typography sx={labelSx}>{t('chem.msds.exportCountries', '수출 대상국')}</Typography>
              <Box sx={valSx}><Typography variant="body2">{selectedItem.exportCountries || ''}</Typography></Box>
            </Box>
            <Box sx={{ display: 'flex' }}>
        ...
        <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 2, mb: 3 }}>
          {[
            ...
            [t('chem.msds.exportCountries', '수출 대상국'), selectedItem.exportCountries],
...
/></Box>
            <Typography sx={labelSx}>{t('chem.msds.exportCountries', '수출 대상국')}</Typography>
            <Box sx={valSx}><TextField fullWidth size="small" value={form.exportCountries} onChange={e => setForm({ ...form, exportCountries: e.target.value })} /></Box>
          </Box>
          <Box sx={{ display: 'flex' }}>
            <Typography sx={labelSx}>{t('chem.msds.fileSize', '파일 크기')}</Typography>
...
/>
          ...
          <Box>
            <Typography variant="body2" fontWeight="bold" sx={{ mb: 0.5, bgcolor: 'grey.200', px: 1.5, py: 0.75, borderRadius: 0.5 }}>{t('chem.msds.exportCountries', '수출 대상국')}</Typography>
            <TextField size="small" fullWidth value={form.exportCountries} onChange={e => setForm({ ...form, exportCountries: e.target.value })} />
          </Box>
          <Box>
        ...
        <Table size="small" sx={{ '& .MuiTableCell-root': { borderRight: '1px solid', borderColor: 'divider' }, '& .MuiTableCell-root:last-child': { borderRight: 'none' } }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={headerCellSx}>{t('chem.msds.exportCountries', '수출 대상국')}</TableCell>
          ...
          <TableBody>
            {items.length > 0 ? items.map((item) => (
              <TableRow key={item.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
                ...
                <TableCell sx={cellBorderSx}>{item.exportCountries || ''}</TableCell>
        ...
        {items.length > 0 ? items.map((item) => (
          <Paper key={item.id} sx={{ p: 2, cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
            ...
            <Typography variant="caption" color="text.secondary">
              {item.itemCode || ''} | v{item.version || ''} | {item.exportCountries || ''}
            </Typography>
          </Paper>
        )) : (
...
export default MsdsProductLatestTab
