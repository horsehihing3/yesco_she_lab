...
const emptyForm = { itemName: '', casNumber: '', version: '', issueDate: '', retireDate: '', retireReason: '', language: '', fileSize: '', msdsType: 'RAW' as const, isLatest: false }

const MsdsRawOldTab: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['msds-raw-old', page],
    queryFn: () => msdsApi.getByType('RAW', false, page, 10),
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['msds-raw-old'] })
  ...
  const handleOpenEdit = (item: Msds) => {
    ...
    setForm({ itemName: item.itemName || '', casNumber: item.casNumber || '', version: item.version || '', issueDate: item.issueDate || '', retireDate: item.retireDate || '', retireReason: item.retireReason || '', language: item.language || '', fileSize: item.fileSize || '', msdsType: 'RAW', isLatest: false })
    setViewMode('edit')
  }
  ...
  // DEV ONLY — 비어있는 항목을 원료 MSDS 구버전 더미데이터로 채움 (입력값 보존)
  const fillTestData = () => setForm(prev => ({
    ...
    retireReason: prev.retireReason || '신규 버전(2.0) 발행으로 폐기',
  }))
        ...
        {items.length > 0 ? items.map((item) => (
          <Paper key={item.id} sx={{ p: 2, cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
            ...
            <Typography variant="caption" color="text.secondary">
              {item.casNumber || ''} | {t('chem.msds.retireDate', '폐기일')}: {item.retireDate || ''}
            </Typography>
          </Paper>
        )) : (
