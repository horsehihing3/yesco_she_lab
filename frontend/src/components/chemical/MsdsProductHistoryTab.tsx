...
const emptyForm = { itemName: '', itemCode: '', version: '', changeType: 'LATEST', changeSummary: '', registeredBy: '', issueDate: '', msdsType: 'PRODUCT' as const }

const MsdsProductHistoryTab: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['msds-product-history', page, keyword],
    queryFn: () => msdsApi.search({ msdsType: 'PRODUCT', keyword: keyword || undefined, page, size: 10 }),
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['msds-product-history'] })
  ...
  const handleOpenEdit = (item: Msds) => {
    ...
    setForm({ itemName: item.itemName || '', itemCode: item.itemCode || '', version: item.version || '', changeType: item.isLatest ? 'LATEST' : 'OLD', changeSummary: item.changeSummary || '', registeredBy: item.registeredBy || '', issueDate: item.issueDate || '', msdsType: 'PRODUCT' })
    setViewMode('edit')
  }
  const handleSave = () => {
        ...
        {items.length > 0 ? items.map((item) => (
          <Paper key={item.id} sx={{ p: 2, cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
            ...
            <Typography variant="caption" color="text.secondary">
              {item.itemCode || ''} | v{item.version || ''} | {item.registeredBy || ''}
            </Typography>
          </Paper>
        )) : (
...
export default MsdsProductHistoryTab
