  ...
  return (
    <Box>
      ...
      <Stack direction="column" spacing={1} sx={{ display: { xs: 'flex', md: 'none' }, mb: 2 }}>
        <ListSearchBar fullWidth placeholder="문서명·관련 인허가 검색" value={searchInput} onChange={setSearchInput} onSearch={applySearch} />
        <TextField select size="small" fullWidth value={filterType} onChange={(e) => setFilterType(e.target.value)}>
          <MenuItem value="all">전체</MenuItem>
          {DOC_TYPES.map((t) => <MenuItem key={t} value={t}>{t}</MenuItem>)}
        </TextField>
