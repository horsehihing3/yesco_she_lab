  ...
  return (
    <Box>
      ...
      <Stack direction="column" spacing={1} sx={{ display: { xs: 'flex', md: 'none' }, mb: 2 }}>
        <ListSearchBar fullWidth placeholder="점검명·시설 검색" value={searchInput} onChange={setSearchInput} onSearch={applySearch} />
        <TextField select size="small" fullWidth value={filterFreq} onChange={(e) => setFilterFreq(e.target.value)}>
          <MenuItem value="all">전체</MenuItem>
          {FREQS.map((f) => <MenuItem key={f} value={f}>{f}</MenuItem>)}
        </TextField>
