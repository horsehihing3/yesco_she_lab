...
const ChemicalInventoryTab: React.FC = () => {
  ...
  const handleOpenEdit = (item: Chemical) => {
    ...
    setForm({ chemicalNameKo: item.chemicalNameKo, chemicalNameEn: item.chemicalNameEn, casNumber: item.casNumber, hazardClass: item.hazardClass, status: item.status, storageLocation: item.storageLocation, storageQuantity: item.storageQuantity, unit: item.unit, maxStorageLimit: item.maxStorageLimit, supplier: item.supplier, department: item.department, handlerName: item.handlerName, emergencyProcedure: item.emergencyProcedure, ghsPictogram: item.ghsPictogram, signalWord: item.signalWord, hazardStatements: item.hazardStatements, precautionaryStatements: item.precautionaryStatements, notes: item.notes })
    setViewMode('edit')
  }
    ...
    return (
      <Box>
          ...
          <Box sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden', mb: 3 }}>
            ...
            <Box sx={rowSx}><Typography sx={dLabelSx}>{t('chem.handler')}</Typography><Box sx={valBorderSx}><Typography variant="body2" sx={{ py: 0.5 }}>{selectedItem.handlerName || ''}</Typography></Box><Typography sx={dLabelSx}>{t('chem.signalWord')}</Typography><Box sx={valSx}><Typography variant="body2" sx={{ py: 0.5 }}>{selectedItem.signalWord || ''}</Typography></Box></Box>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
                    ...
                    <TableCell align="center" sx={{ fontFamily: 'monospace', color: item.maxStorageLimit && (item.storageQuantity ?? 0) > item.maxStorageLimit ? 'error.main' : 'inherit' }}>
