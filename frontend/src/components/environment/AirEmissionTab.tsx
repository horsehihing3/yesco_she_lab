  ...
  const renderListView = () => (
    <Box>
      <Box sx={{ display: 'flex', gap: 1, mb: 2, alignItems: 'center', flexWrap: 'wrap' }}>
        <TextField size="small" placeholder={t('environment.searchAirEmission')} value={searchText}
        ...
        <Table size="small" sx={{ '& .MuiTableCell-root': { borderRight: '1px solid', borderColor: 'grey.300' }, '& .MuiTableCell-root:last-child': { borderRight: 'none' } }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.300', wordBreak: 'keep-all' }}>{t('environment.facility')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.300', wordBreak: 'keep-all' }}>{t('environment.pollutant')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.300', wordBreak: 'keep-all' }}>{t('environment.emissionConcentration')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.300', wordBreak: 'keep-all' }}>{t('environment.emissionStandard')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.300', wordBreak: 'keep-all' }}>{t('environment.compliance')}</TableCell>
          ...
          <TableBody>
            {data && data.content.length > 0 ? data.content.map((item, idx) => (
              <TableRow key={item.id} hover sx={{ cursor: 'pointer' }} onClick={() => handleRowClick(item)}>
                ...
                <TableCell sx={{ borderRight: 1, borderColor: 'grey.300' }}>{item.facility || ''}</TableCell>
                <TableCell align="center" sx={{ borderRight: 1, borderColor: 'grey.300' }}>{getPollutantLabel(item.pollutant || '') || ''}</TableCell>
                <TableCell align="center" sx={{ borderRight: 1, borderColor: 'grey.300' }}>{item.emissionConcentration != null ? `${item.emissionConcentration} ${getEmissionUnitLabel(item.unit || '')}` : ''}</TableCell>
                <TableCell align="center" sx={{ borderRight: 1, borderColor: 'grey.300' }}>{item.emissionStandard != null ? `${item.emissionStandard} ${getEmissionUnitLabel(item.unit || '')}` : ''}</TableCell>
                <TableCell align="center" sx={{ borderRight: 1, borderColor: 'grey.300' }}>{getComplianceChip(item.compliance)}</TableCell>
    ...
    <Box>
      ...
      <Box sx={{ display: { xs: 'none', md: 'block' }, border: 1, borderColor: 'grey.300', borderRadius: 1, overflow: 'hidden' }}>
        ...
        <Box sx={rowSx}>
          ...
          <Box sx={cellRSx}>
            <FormControl fullWidth size="small">
              <Select value={formData.pollutant || ''} onChange={(e: SelectChangeEvent) => setFormData({ ...formData, pollutant: e.target.value })}>
        ...
        <Box sx={rowSx}>
          ...
          <Box sx={cellRSx}>
            <FormControl fullWidth size="small">
              <Select value={formData.compliance || ''} onChange={(e: SelectChangeEvent) => setFormData({ ...formData, compliance: e.target.value })}>
