...
interface Props {
  entityId: number
  /** 백엔드 컨트롤러의 RequestMapping 경로 — 예: '/messages', '/qna' */
  basePath: string
  /** React Query 캐시 키의 1차 키 — 엔티티별 고유 식별자 */
  queryKey: string
}
...
const EntityCommentsSection: React.FC<Props> = ({ entityId, basePath, queryKey }) => {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get<ApiResponse<EntityComment[]>>(`${basePath}/${entityId}/comments`)
  ...
  const createMut = useMutation({
    mutationFn: async (body: Partial<EntityComment>) => {
      const res = await axiosInstance.post<ApiResponse<EntityComment>>(`${basePath}/${entityId}/comments`, body)
      return res.data.data
    },
    onSuccess: () => {
  ...
  const updateMut = useMutation({
    mutationFn: async ({ id, content }: { id: number; content: string }) => {
      const res = await axiosInstance.put<ApiResponse<EntityComment>>(`${basePath}/comments/${id}`, { content })
      return res.data.data
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [queryKey, entityId] })
